...
```bash
scripts/spawn.bash --nodes 5 --home $HOME/.malachite --app arc-malachitebft-test-app
```
...
```
The script will build the `arc-malachitebft-test-app` application by default, unless the name is
...
`--app`: set the name of the application that will be built. The default is `arc-malachitebft-test-app`.
...
`APP_BINARY=arc-malachitebft-test-app make -C scripts`: set the name of the application that will be
built and executed. The default is `arc-malachitebft-test-app`.

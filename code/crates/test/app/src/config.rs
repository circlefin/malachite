...
pub use malachitebft_app_channel::app::config::{
    ConsensusConfig, LoggingConfig, MetricsConfig, RuntimeConfig, TestConfig, ValueSyncConfig,
};
...
pub struct Config {
    ...
    ///
    /// When present and active, the node can exhibit configurable Byzantine
    /// faults such as vote/proposal equivocation, vote/proposal dropping, or
    /// amnesia-style lock ignoring.
...
mod tests {
    ...
    fn parse_default_config_file() {
        ...
        let _config = toml::from_str::<Config>(file).unwrap();

        let tmp_file = std::env::temp_dir().join("config-test.toml");
        std::fs::write(&tmp_file, file).unwrap();

        let _config = load_config(&tmp_file, None).unwrap();

...
pub type ProposedValueKey = (HeightKey, RoundKey, BlockHashKey);
...
// #[derive(Copy, Clone, Debug)]
// struct HeightRoundValueIdKey;
//
// impl redb::Value for HeightRoundValueIdKey {
//     type SelfType<'a> = (Height, Round, BlockHash);
//
//     type AsBytes<'a> = Vec<u8>;
//
//     fn fixed_width() -> Option<usize> {
//         use core::mem::size_of;
//
//         let height_size = HeightKey::fixed_width()?;
//         Some(height_size + size_of::<u32>() * 2 + size_of::<[u8; 32]>())
//     }
//
//     fn from_bytes<'a>(data: &'a [u8]) -> Self::SelfType<'a>
//     where
//         Self: 'a,
//     {
//         let (height, round, hash) = <(HeightKey, i64, [u8; 32]) as redb::Value>::from_bytes(data);
...
//         Self: 'b,
//     {
//         <(HeightKey, i64, [u8; 32]) as redb::Value>::as_bytes(&(
//             value.0,
//             value.1.as_i64(),
//             *value.2.as_bytes(),
//         ))

...
impl Handler {
    ...
    pub(crate) fn register_dialed_peer_id(&mut self, peer_id: PeerId) {
    ...
    pub(crate) fn has_already_dialed(&self, connection_data: &ConnectionData) -> bool {
        connection_data
            .get_peer_id()
            .as_ref()
            ...
            || self
                .dialed_multiaddrs
                .contains(&connection_data.get_multiaddr())
    }

    pub(crate) fn register_pending_connection(
        &mut self,
    ...
    pub(crate) fn remove_pending_connection(
        &mut self,
        connection_id: &ConnectionId,
    ...
    pub(crate) fn remove_connection_type(&mut self, peer_id: &PeerId) -> Option<ConnectionType> {
        self.connections_types.remove(peer_id)
    }

    pub(crate) fn register_requested_peer_id(&mut self, peer_id: PeerId) {
        self.requested_peer_ids.insert(peer_id);
    }

    pub(crate) fn has_already_requested(&self, peer_id: &PeerId) -> bool {
        self.requested_peer_ids.contains(peer_id)
    }

    pub(crate) fn register_pending_request(&mut self, request_id: OutboundRequestId) {
        self.pending_requests.insert(request_id);
    }

    pub(crate) fn remove_pending_request(&mut self, request_id: &OutboundRequestId) {

...
impl<S: ScoringStrategy> PeerScorer<S> {
    ...
    pub fn new(initial_score: Score, strategy: S) -> Self {
        Self {
            state: HashMap::new(),
            initial_score,
            strategy,
    ...
    ///
    /// This is used when a peer is first encountered or when a peer's score is reset due to inactivity.
    pub fn initial_score(&self) -> Score {
        self.initial_score
    }
    ...
    pub fn update_score(&mut self, peer_id: PeerId, result: SyncResult) -> Score {
        let initial_score = self.initial_score();

        let peer_state = self
            .state
            .entry(peer_id)
            .or_insert_with(|| PeerState::new(initial_score));
    ...
    pub fn get_score(&self, peer_id: &PeerId) -> Score {
        self.state
            .get(peer_id)
            .map(|p| p.score)
            .unwrap_or_else(|| self.initial_score())
    }
...
mod tests {
    ...
    fn arb_scorer_variant(u: &mut Unstructured) -> Result<(ScorerVariant, Duration)> {
        ...
        let scorer = if u.arbitrary()? {
            ...
            ScorerVariant::Ema(PeerScorer::new(INITIAL_SCORE, strategy))
        } else {
            eprintln!("Testing with Credit strategy");
            let strategy = arb_credit_strategy(u, slow_threshold)?;
            ScorerVariant::Credit(PeerScorer::new(INITIAL_SCORE, strategy))
        };
        ...
        fn for_scorer(scorer: &ScorerVariant) -> Self {
            match scorer {
                ...
                ScorerVariant::Credit(s) => {
                    ...
                    StrategyState::Credit(s.strategy.initial_credit())
                }
            }
        }
    }
    ...
    fn slow_responses_decrease_high_score() {
        arbtest(|u| {
            ...
            let init_score = 1.0; // Start with a high score to test the penalty effect
            let mut state = StrategyState::for_scorer(&scorer);
            ...
            assert!(
                update_score < init_score,
                "Slow response should decrease high score: {init_score} -> {update_score}",
            );
    ...
    fn peer_selection_is_deterministic() {
        arbtest(|u| {
            ...
            {
                let strategy = arb_ema_strategy(u, slow_threshold)?;
                let mut scorer1 = PeerScorer::new(INITIAL_SCORE, strategy);
                let mut scorer2 = PeerScorer::new(INITIAL_SCORE, strategy);
            ...
            // Test with Credit strategy
            {
                let strategy = arb_credit_strategy(u, slow_threshold)?;
                let mut scorer1 = PeerScorer::new(INITIAL_SCORE, strategy.clone());
                let mut scorer2 = PeerScorer::new(INITIAL_SCORE, strategy);
    ...
    fn monotonic_score_updates() {
        arbtest(|u| {
            let slow_threshold = arb_slow_threshold(u)?;
            let strategy = arb_credit_strategy(u, slow_threshold)?;
            let scorer = ScorerVariant::Credit(PeerScorer::new(INITIAL_SCORE, strategy));

...
pub fn precommit<C>(state: State<C>, value: C::Value) -> Transition<C>
where
    C: Consensus,
{
    let message = Message::precommit(state.round, Some(value.id()), C::DUMMY_ADDRESS);
...
pub fn set_valid_value<C>(state: State<C>, value: C::Value) -> Transition<C>
where
    C: Consensus,
{
...
///
/// Ref: L49
pub fn commit<C>(state: State<C>, round: Round, value: C::Value) -> Transition<C>
where
    C: Consensus,
{
    // Check that we're locked on this value
